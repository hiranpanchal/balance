...
function Stars({ rating }: { rating: number }) {
  return (
    <div className="flex gap-1" aria-label={`${rating} out of 5 stars`}>
      {[1, 2, 3, 4, 5].map((n) => (
        <span key={n} aria-hidden className="text-[22px] leading-none" style={{ color: n <= rating ? "#B28B5D" : "#D1C4B0" }}>
          ★
        </span>
      ))}
    </div>
    ...
    <>
          ...
          {avgRating && (
            <div className="mt-8 flex flex-col items-center gap-2">
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((n) => (
                  <span key={n} className="text-[32px] leading-none" style={{ color: "#B28B5D" }}>★</span>
                ))}
              </div>
      ...
      <section className="py-12 md:py-16 px-6 md:px-12">
        <div className="max-w-[1200px] mx-auto">
          {reviews.length === 0 ? (
            <div className="text-center py-16 text-teal/50 text-[15px]">
